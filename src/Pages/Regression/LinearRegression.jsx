...
export default function LinearRegression() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('http://127.0.0.1:8080/regression/linear-regression', {
        X: xValues,
        y: yData,
        ...
        random_state: formData.random_state ? parseInt(formData.random_state, 10) : 42
      });
              ...
              <div className="card-body">
                <div className="d-flex align-items-center">
                  <div className="ms-3">
                    ...
                    <p className="card-text text-muted mb-1">R squared value: <strong>{responseData.r2}</strong></p>
                    <p className="card-text text-muted mb-0">MSE value: <strong>{responseData.mse}</strong></p>
                    {responseData.y_new && <p className="card-text text-muted mb-0">y_new value: <strong>{responseData.y_new}</strong></p>}
                  </div>
                </div>
              </div>
            </div>
